...
class _MessSecScreenState extends State<MessSecScreen> {
  ...
  Future<void> _saveFinalListToFirestore() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Final list sent to Purchase Manager')),
      );
    } catch (e) {
        ...
        return Column(
          children: [
            ...
            _greenButton('Verify & Forward', () async {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Delivery verified and forwarded!'),
                ),
              );
            }),
          ],
          ...
          children: [
            _card('Ordered List', Icons.shopping_cart, [
              ...
              ...ordered.map((e) {
                return ListTile(
                  title: Text(e['item']!),
                  subtitle: Text('${e['qty']} • ${e['brand']}'),
                );
              }),
            ]),

            _card('Final List', Icons.playlist_add_check, [
              _greenButton('Move All to Final', () {
                setState(() {
            ...
            _card('Received List', Icons.inventory, [
              _buildReceivedListStream(),
            ]),

            _card('Student OTP Generation', Icons.lock, [
              _tf(_studentNumberController, 'Student Number'),

              _greenButton('Generate OTP', () {
                final studentNumber = _studentNumberController.text;

                if (studentNumber.isNotEmpty) {
                  OTPStore.generateOTP(studentNumber);
            ...
            ElevatedButton.icon(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => OthersTabScreen()),
                );
              },
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          enabledBorder: OutlineInputBorder(
            borderSide: BorderSide(color: accentGreen.withOpacity(0.4)),
            borderRadius: BorderRadius.circular(12),
          ),
        ),
      ),
  ...
  Widget _greenButton(String t, VoidCallback o) {
    return SizedBox(
