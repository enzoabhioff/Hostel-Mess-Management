...
class _PurchaseManagerScreenState extends State<PurchaseManagerScreen> {
    ...
    if (_receivedFormKey.currentState!.validate()) {
      setState(() {
        _receivedToday.add({
          "item": _receivedItemController.text.trim(),
          "qty": int.tryParse(_receivedQtyController.text.trim()) ?? 0,
          "brand": _receivedBrandController.text.trim().isEmpty
              ? "—"
              : _receivedBrandController.text.trim(),
          "time": DateTime.now().toIso8601String(),
        });
      });
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Received item added")));
  ...
  Future<void> _submitAllEntries() async {
    if (_receivedToday.isEmpty && _issuesReported.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("No entries to submit")));
      return;
    }
    ...
    try {
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text(
            "Deliveries sent to Mess Secretary successfully.",
          ),
          actions: [
            TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Purchase Manager Dashboard"),
        centerTitle: true,
        backgroundColor: const Color(0xFF027855),
        actions: [
          IconButton(
        ...
        child: Column(
          ...
          children: [
            const Center(
              child: Column(
                children: [
                  Icon(Icons.inventory, size: 80, color: Color(0xFF027855)),
                  SizedBox(height: 10),
                  Text(
            ...
            _sectionTitle("Pending / Ordered Items"),
            _buildOrderedItems(),

            const SizedBox(height: 30),

            _sectionTitle("Record Received Items"),
            _receivedForm(),

            const SizedBox(height: 30),

            _sectionTitle("Report Missed / Faulty Items"),
            _issueForm(),
            ...
            Center(
              child: ElevatedButton.icon(
                icon: const Icon(Icons.send),
                label: const Text("Submit All Entries"),
                onPressed: _submitAllEntries,
              ),
            ),
          ],
      ...
      child: Card(
        child: Padding(
          ...
          child: Column(
            children: [
              TextFormField(
                controller: _receivedItemController,
              ...
              TextFormField(
                controller: _receivedBrandController,
      ...
      child: Card(
        child: Padding(
          ...
          child: Column(
            children: [
              TextFormField(
                controller: _issueItemController,
                decoration: const InputDecoration(labelText: "Item Name"),
              ),
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: "Qty Affected"),
                keyboardType: TextInputType.number,
              ),
